...
uniform float colores;
uniform float transparencia;
uniform float translucido;
...
vec3 convertHSVToRGB(in float hue, in float saturation, in float lightness) {
 // hue = hue + translucido *2.0;
  float chroma = lightness * saturation * colores;
  float hueDash = hue / 60.0;
  float x = chroma * (1.0 - abs(mod(hueDash, 1.0) - 1.0));
  vec3 hsv = vec3(0.0 + translucido * 0.5);
...
void main()
{
    ...
    vec4 newPosition = vec4(gPosition.x, gPosition.y + verticalScale * sample.a, gPosition.z * 1.8 * transparencia*0.2, 1.0);
